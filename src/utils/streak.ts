...
export type StreakState = {
  ...
  creditedAsRestDay: boolean;
  requiresWorkoutToday: boolean;
...
export const emptyStreakState: StreakState = {
  ...
  creditedAsRestDay: false,
  requiresWorkoutToday: false,
  deloadActive: false,
  deloadSince: null,
};
...
export const creditStreakDay = (state: StreakState, today: Date = new Date(), isRestDay = false): StreakState => {
  if (state.deloadActive) return state;
  ...
  return {
    ...
    creditedAsRestDay: isRestDay,
    requiresWorkoutToday: false,
  };
};

export const invalidateTodayCredit = (state: StreakState, today: Date = new Date()): StreakState => {
  if (state.lastCreditedDate !== dateKey(today)) return state;
  return {
    ...state,
    streak: Math.max(0, state.streak - 1),
    lastCreditedDate: previousDayKey(today),
    creditedAsRestDay: false,
    requiresWorkoutToday: true,
  };
};

export const invalidateRestDayCredit = invalidateTodayCredit;

export const invalidateWorkoutCredit = invalidateTodayCredit;

export const restoreRestDayCredit = (state: StreakState, today: Date = new Date()): StreakState => {
  if (!state.requiresWorkoutToday || state.lastCreditedDate !== previousDayKey(today)) return state;
  return {
    ...state,
    streak: state.streak + 1,
    lastCreditedDate: dateKey(today),
    creditedAsRestDay: true,
    requiresWorkoutToday: false,
  };
};
