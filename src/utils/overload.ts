...
export const estimateOneRepMax = (load: number, reps: number, rpe?: number, rir?: number) => {
  ...
  // Epley is useful for normal working sets, but becomes noisy at very high
  // rep counts. Keep the estimate inside the range where the formula is sane.
  effectiveReps = Math.min(Math.max(effectiveReps, 1), 15);
  if (effectiveReps <= 1) return Math.round(load * 10) / 10;
  return Math.round(load * (1 + effectiveReps / 30) * 10) / 10;
};

const loadInKg = (value: unknown, unit?: string) => {
  const load = parseFloat(String(value ?? "")) || 0;
  return unit === "lb" ? load / 2.20462262 : load;
};
...
const bestPerExercise = (sets: any[]): Record<string, ExerciseStrength> => {
  ...
  (sets || []).forEach((set) => {
    ...
    const load = loadInKg(set?.load, set?.loadUnit);
...
export const getRoutineSessions = (history: any[], routineId?: string | null, routineName?: string | null) => {
  ...
  return (history || [])
    ...
    .sort((a, b) => {
      ...
      const aTime = Number(a.session.createdAt || a.session.id);
      const bTime = Number(b.session.createdAt || b.session.id);
      if (Number.isFinite(aTime) && Number.isFinite(bTime) && aTime !== bTime) return aTime - bTime;

      // Legacy sessions are stored newest-first and only have a calendar date.
...
export const computeRoutineStrength = (
  ...
  return sessions.map((session) => {
    ...
    const ratios = Object.values(baseline)
      .map((baselineExercise) => {
        const current = bests[baselineExercise.key];
        return current && baselineExercise.best > 0 ? current.best / baselineExercise.best : null;
      })
