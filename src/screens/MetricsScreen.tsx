...
import { computeRoutineStrength, describeRoutineStrength, estimateOneRepMax } from "../utils/overload";
import { useUserProfile } from "../context/UserProfileContext";
import { SkillFocus, TrainingModality, TrainingStyle, UserProfile, WeightUnit } from "../models/userProfile";

const WORKOUT_HISTORY_KEY = "@workout_history";
const FAVORITE_EXERCISES_KEY = "@favorite_exercises";
const MUSCLE_GOALS_KEY = "@muscle_weekly_goals";
const COMPLETED_PROGRAMMED_GOALS_KEY = "@completed_programmed_goals";

const createProfileVolumeTargets = (profile: UserProfile): Record<string, number> => {
  const targets: Record<string, number> = {};
  muscleGroups.forEach((muscle) => { targets[muscle.name] = 0; });
  const base = profile.trainingDays <= 2 ? 6 : profile.trainingDays >= 5 ? 12 : 9;
  const focus = profile.trainingStyle === "hypertrophy" ? 3 : profile.trainingStyle === "strength" ? -2 : 0;
  const set = (names: string[], value: number) => names.forEach((name) => { if (name in targets) targets[name] = Math.max(0, value); });
  if (profile.modality !== "calisthenics") set(["Chest", "Back", "Quads", "Hamstrings", "Glutes"], base + focus);
  if (profile.modality !== "weights") set(["Chest", "Back", "Shoulders", "Core"], base + (profile.trainingStyle === "skills" ? -1 : focus));
  set(["Biceps", "Triceps"], Math.max(4, base - 3));
  return targets;
};
...
const MUSCLE_FILTERS = muscleGroups.map((muscle) => muscle.id);
const skillHoldTarget = (progression: string) => progression === "Full" ? 8 : progression === "Straddle" ? 10 : progression === "Advanced tuck" ? 12 : 15;
...
function generateAutomaticGoals(profile: UserProfile, history: any[]) {
  type GoalBucket = "weights" | "calisthenics";
  type ExerciseStats = {
    name: string;
  ...
  const findMatch = (value: string) => {
    ...
    return exerciseLibrary.find((exercise) =>
      [exercise.name, exercise.id, exercise.id?.replace(/-/g, " ")]
        .filter(Boolean)
        .some((candidate) => normalizeName(candidate as string) === normalized)
    );
  };
  const toKg = (value: number, unit: string) =>
    unit === "lb" || unit === "lbs" ? value / 2.20462262 : value;
  const displayUnit = profile.weightUnit || "kg";
  const fromKg = (value: number) => displayUnit === "lb" ? value * 2.20462262 : value;
  const weightStep = displayUnit === "lb" ? 5 : 2.5;
  const stats = new Map<string, ExerciseStats>();

  history.forEach((session) => {
    const sessionDate = new Date(session.date || 0).getTime() || 0;
    (session.sets || []).forEach((setObj: any) => {
      if (!setObj.exercise) return;
      const name = String(setObj.exercise);
      const key = normalizeName(name);
      const match = findMatch(name);
      const isCalisthenics = match?.modality === "calisthenics" || match?.type === "skill-static" || match?.type === "skill-dynamic";
      const load = toKg(parseFloat(setObj.load || "0") || 0, String(setObj.loadUnit || "kg").toLowerCase());
      const reps = parseInt(setObj.reps || "0", 10) || 0;
      if (reps <= 0) return;
      ...
      existing.isCalisthenics = existing.isCalisthenics || !!isCalisthenics;
      existing.lastDate = Math.max(existing.lastDate, sessionDate);
      if (setObj.isHold || match?.type === "skill-static") {
        existing.maxHoldSeconds = Math.max(existing.maxHoldSeconds, reps);
      } else {
        existing.maxWeightKg = Math.max(existing.maxWeightKg, load);
        if (load <= 0) existing.maxReps = Math.max(existing.maxReps, reps);
      }
      stats.set(key, existing);
    });
  });

  // Before the first workout, onboarding is the only useful evidence we have.
  // Turn it into modest, explicit first targets instead of an empty goals screen.
  if (stats.size === 0) {
    const initial: any[] = [];
    const addLift = (name: string, value?: number) => {
      if (!value) return;
      const target = Math.round((value + (displayUnit === "lb" ? 5 : 2.5)) * 10) / 10;
      initial.push({ id: `baseline-lift-${name}`, title: `First checkpoint: ${name}`, detail: `Use your saved ${value} ${displayUnit} max as a reference. Work up to ${target} ${displayUnit} only when your technique is ready.`, current: value, target, unit: displayUnit, bucket: "weights" as GoalBucket, lastDate: 0 });
    };
    const addRep = (name: string, value?: number) => {
      if (!value) return;
      initial.push({ id: `baseline-reps-${name}`, title: `First checkpoint: ${name}`, detail: `Add 1 controlled rep to the clean baseline you saved in onboarding.`, current: value, target: value + 1, unit: "reps", bucket: "calisthenics" as GoalBucket, lastDate: 0 });
    };
    if (profile.modality !== "calisthenics") {
      addLift("Squat", profile.squat1RM); addLift("Bench press", profile.bench1RM); addLift("Deadlift", profile.deadlift1RM);
    }
    if (profile.modality !== "weights") {
      addRep("Pull-ups", profile.pullUps); addRep("Push-ups", profile.pushUps); addRep("Dips", profile.dips);
      if (profile.trainingStyle === "skills" || profile.modality === "hybrid") {
        const hold = profile.frontLeverProgression && profile.frontLeverProgression !== "Not yet" ? skillHoldTarget(profile.frontLeverProgression) : 0;
        if (hold) initial.push({ id: "baseline-front-lever", title: "Own your front lever", detail: `Accumulate clean ${hold}-second holds at your saved progression before moving on.`, current: 0, target: hold * 3, unit: "sec total", bucket: "calisthenics" as GoalBucket, lastDate: 0 });
      }
    }
    return initial.slice(0, 4);
  }

  const weightCandidates: any[] = [];
  const calisthenicsCandidates: any[] = [];
  stats.forEach((item) => {
    const weight = fromKg(item.maxWeightKg);
    const nextWeight = Math.round((weight + weightStep) * 10) / 10;
    const nextReps = item.maxReps + (item.maxReps < 5 ? 2 : 1);
    const nextHold = item.maxHoldSeconds + 5;
    const weightGoal = {
      id: `auto-weight-${normalizeName(item.name)}-${nextWeight}`,
      title: `Add weight: ${item.name}`,
      detail: `Beat your logged best by ${weightStep} ${displayUnit}.`,
      current: Math.round(weight * 10) / 10,
      target: nextWeight,
      unit: displayUnit,
      bucket: "weights" as GoalBucket,
      lastDate: item.lastDate,
    };
    const calisthenicsGoal = item.maxHoldSeconds > 0
      ? {
          id: `auto-hold-${normalizeName(item.name)}-${nextHold}`,
          title: `Hold longer: ${item.name}`,
          detail: `Add 5 seconds while keeping clean position.`,
          current: item.maxHoldSeconds,
          target: nextHold,
          unit: "sec",
          bucket: "calisthenics" as GoalBucket,
          lastDate: item.lastDate,
        }
      : item.maxWeightKg > 0 && item.isCalisthenics
      ? {
          id: `auto-weighted-cali-${normalizeName(item.name)}-${nextWeight}`,
          title: `Add load: ${item.name}`,
          detail: `Progress your weighted calisthenics by ${weightStep} ${displayUnit}.`,
          current: Math.round(weight * 10) / 10,
          target: nextWeight,
          unit: displayUnit,
          bucket: "calisthenics" as GoalBucket,
      ...
      : {
          id: `auto-reps-${normalizeName(item.name)}-${nextReps}`,
          title: `More clean reps: ${item.name}`,
          detail: `Reach ${nextReps} controlled bodyweight reps.`,
          current: item.maxReps,
          target: nextReps,
          unit: "reps",
          bucket: "calisthenics" as GoalBucket,
          lastDate: item.lastDate,
    ...
    if (profile.modality === "weights") {
      if (item.maxWeightKg > 0 && !item.isCalisthenics) weightCandidates.push(weightGoal);
    } else if (profile.modality === "calisthenics") {
      if (item.maxHoldSeconds > 0 || item.maxReps > 0 || (item.maxWeightKg > 0 && item.isCalisthenics)) {
        calisthenicsCandidates.push(calisthenicsGoal);
      }
    } else {
      if (item.maxWeightKg > 0 && !item.isCalisthenics) weightCandidates.push(weightGoal);
      if (item.maxHoldSeconds > 0 || item.maxReps > 0 || (item.maxWeightKg > 0 && item.isCalisthenics)) {
  ...
  weightCandidates.sort((a, b) => b.lastDate - a.lastDate);
  calisthenicsCandidates.sort((a, b) => b.lastDate - a.lastDate);
  if (profile.modality === "weights") return weightCandidates.slice(0, 5);
  if (profile.modality === "calisthenics") return calisthenicsCandidates.slice(0, 5);

  const mixed: any[] = [];
  for (let index = 0; mixed.length < 5 && (index < weightCandidates.length || index < calisthenicsCandidates.length); index += 1) {
    if (weightCandidates[index]) mixed.push(weightCandidates[index]);
    if (calisthenicsCandidates[index] && mixed.length < 5) mixed.push(calisthenicsCandidates[index]);
  }
...
export function MetricsScreen() {
  ...
  const [completedGoalTitles, setCompletedGoalTitles] = useState<string[]>([]);
  ...
  const [prUnit, setPrUnit] = useState<WeightUnit>(profile?.weightUnit || "kg");

  const formatProfileHeight = () => {
    if (!profile?.height) return "--";
    if (profile.heightUnit === "ft") {
      return `${profile.height} ft ${profile.heightInches || 0} in`;
    }
    return `${profile.height} cm`;
  };

  const changePrUnit = (nextUnit: WeightUnit) => {
    if (nextUnit !== prUnit && targetWeightInput) {
      const raw = parseFloat(targetWeightInput);
      if (Number.isFinite(raw)) {
        const converted = prUnit === "kg" ? raw * 2.20462262 : raw / 2.20462262;
        setTargetWeightInput(String(Math.round(converted * 10) / 10));
      }
    }
    setPrUnit(nextUnit);
  };
  ...
  useEffect(() => {
    const loadScreenData = async () => {
      try {
        const [storedHistory, storedFavorites, storedMuscleGoals, storedCompletedGoals] = await Promise.all([
          AsyncStorage.getItem(WORKOUT_HISTORY_KEY),
          AsyncStorage.getItem(FAVORITE_EXERCISES_KEY),
          AsyncStorage.getItem(MUSCLE_GOALS_KEY),
          AsyncStorage.getItem(COMPLETED_PROGRAMMED_GOALS_KEY),
        ]);

        if (storedHistory) setHistory(JSON.parse(storedHistory));
        if (storedFavorites) setFavorites(JSON.parse(storedFavorites));
        if (storedCompletedGoals) setCompletedGoalIds(JSON.parse(storedCompletedGoals));

        if (storedMuscleGoals) {
          setWeeklyGoals(JSON.parse(storedMuscleGoals));
        } else if (profile) {
          setWeeklyGoals(createProfileVolumeTargets(profile));
        } else {
          const initialGoals: Record<string, number> = {};
          muscleGroups.forEach((muscle) => { initialGoals[muscle.name] = 8; });
  ...
  const programmedGoals = useMemo<any[]>(() => [], []);

  // Keep this hook above the loading return so hook order never changes between
  // the loading render and the fully loaded screen.
  useEffect(() => {
    const newlyCompleted = programmedGoals.filter(
      (goal) => goal.current >= goal.target && !completedGoalIds.includes(goal.id)
    );

    if (newlyCompleted.length > 0) {
      const ids = [...completedGoalIds, ...newlyCompleted.map((goal) => goal.id)];
      setCompletedGoalIds(ids);
      setCompletedGoalTitles(newlyCompleted.map((goal) => goal.title));
      setShowGoalCompletion(true);
      AsyncStorage.setItem(COMPLETED_PROGRAMMED_GOALS_KEY, JSON.stringify(ids)).catch((error) => {
        console.error("Failed to save completed goal milestones", error);
      });
    }
  }, [programmedGoals, completedGoalIds]);
  ...
  const displayedMuscles = showAllMuscles
    ? sortedMuscles
    : sortedMuscles.filter(([muscle, score]) => score > 0 || weeklyGoals[muscle] !== undefined);
  ...
  const convertWeight = (value: number, from: WeightUnit = "kg", to: WeightUnit = prUnit) => {
    if (from === to) return value;
    return from === "kg" ? value * 2.20462262 : value / 2.20462262;
  };

  const formatWeight = (value: number, from: WeightUnit = "kg") => `${Math.round(convertWeight(value, from, prUnit) * 10) / 10} ${prUnit}`;
  ...
  history.forEach((session) => {
    if (!session.sets) return;
    session.sets.forEach((setObj: any) => {
      const currentWeight = convertWeight(parseFloat(setObj.load || "0") || 0, setObj.loadUnit || "kg", "kg");
      ...
      if (currentWeight > 0) {
        const computed1RM = estimateOneRepMax(currentWeight, currentReps, Number(setObj.rpe) || undefined, Number(setObj.rir) || undefined);
  ...
  const getPrProgressionHistory = (
    ...
    const targetWeight = convertWeight(parseFloat(weightFilterVal), prUnit, "kg");
    ...
    const chronologicalHistory = [...history].sort((a, b) => {
      ...
      const aTime = Number(a.createdAt || a.id);
      const bTime = Number(b.createdAt || b.id);
    ...
    chronologicalHistory.forEach((session) => {
      ...
      session.sets.forEach((setObj: any) => {
        ...
        const w = convertWeight(parseFloat(setObj.load || "0") || 0, setObj.loadUnit || "kg", "kg");
        ...
        if (!isHold && hasRepTarget && r !== targetReps) return;
        ...
        if (isHold) {
          ...
          const computed1RM = estimateOneRepMax(w, r, Number(setObj.rpe) || undefined, Number(setObj.rir) || undefined);
          if (mode === "maxWeight") {
          ...
          if (shouldUpdate) {
            dailyBestMap[session.date] = { date: session.date, weight: w, reps: r, estimated1RM: computed1RM, isHold: false, sourceUnit: "kg" };
          }
          return;
        }

        if (shouldUpdate) {
          dailyBestMap[session.date] = { date: session.date, weight: w, reps: r, isHold: true, sourceUnit: "kg" };
        }
      });
    });
  ...
  if (loading) {
    return (
      <SafeAreaView edges={["top"]} style={styles.screenBackground}>
  ...
  const overloadPath = overloadPointCoords.length
    ...
    : "";
  const activeProgrammedGoals = programmedGoals.filter(
    (goal) => goal.current < goal.target && !completedGoalIds.includes(goal.id)
  );
  ...
  return (
    <SafeAreaView edges={["top"]} style={styles.screenBackground}>
      <ScrollView contentContainerStyle={styles.container}>
        {profile && (
          <View style={styles.profileCard}>
            <View style={styles.profileCardHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.profileEyebrow}>TRAINING PROFILE</Text>
                <Text style={styles.profileTitle}>{profile.modality === "weights" ? "Weightlifting" : profile.modality === "calisthenics" ? "Calisthenics" : "Hybrid training"}</Text>
                <Text style={styles.profileSubtitle}>{profile.trainingStyle === "mixed" ? "Balanced mix" : profile.trainingStyle.charAt(0).toUpperCase() + profile.trainingStyle.slice(1)}  •  {profile.weight || "--"} {profile.weightUnit}  •  {formatProfileHeight()}</Text>
              </View>
              <Button variant="outline" size="small" onPress={openProfileEditor}>Edit</Button>
            </View>
            <View style={styles.profileSignals}><Text style={styles.profileSignal}>Plan: {profile.trainingDays || 3} sessions</Text><Text style={styles.profileSignal}>{profile.weight ? `Baseline: ${profile.weight} ${profile.weightUnit}` : "Add a baseline"}</Text></View>
          </View>
        )}

        {false && <View style={styles.goalsPanel}>
          <View style={styles.recommendationHeader}>
            <Pressable style={styles.goalsHeaderToggle} onPress={() => setIsGoalsCollapsed((collapsed) => !collapsed)}>
              <View style={{ flex: 1 }}>
                <Text style={styles.profileEyebrow}>GOALS</Text>
                <Text style={styles.recommendationTitle}>Performance targets</Text>
              </View>
              <Text style={styles.goalsCollapseIcon}>{isGoalsCollapsed ? "＋" : "−"}</Text>
            </Pressable>
            <Text style={styles.recommendationCount}>{activeProgrammedGoals.length}</Text>
          </View>

          {!isGoalsCollapsed && (
            <>
              <View style={styles.goalTipsBanner}>
                <Text style={styles.goalTipsTitle}>Progress updates automatically</Text>
                <Text style={styles.goalTipsText}>
                  Your first targets use onboarding baselines. Once you log training, they become exercise-specific next steps that update automatically.
                </Text>
              </View>

              {activeProgrammedGoals.length === 0 ? (
                <Text style={styles.emptyText}>
                  {programmedGoals.length > 0
                    ? "All automatic goals are complete. Keep training to generate your next targets."
                    : "Complete a few workouts to generate automatic profile-based goals."}
                </Text>
              ) : (
                activeProgrammedGoals.map((goal) => {
                  const progress = goal.target > 0
                    ? Math.min(100, Math.round((goal.current / goal.target) * 100))
                    : 0;
                  return (
                    <View key={goal.id} style={styles.programmedGoalRow}>
                      <View style={styles.programmedGoalHeader}>
                        <Text style={styles.recommendationItemTitle}>{goal.title}</Text>
                        <Text style={styles.goalValue}>{goal.current} / {goal.target} {goal.unit}</Text>
                      </View>
                      <View style={styles.goalTrack}>
                        <View style={[styles.goalFill, { width: `${progress}%` }]} />
                      </View>
                      <Text style={styles.recommendationDetail}>{goal.detail}</Text>
                    </View>
                  );
                })
              )}
        ...
        {false && <View style={styles.sectionContainer}>
          <View style={styles.sectionHeaderRow}>
            <Text style={styles.sectionTitle}>Weekly Target Progress (Sun - Sat)</Text>
            <Pressable
              style={styles.editGoalsHeaderButton}
              onPress={() => {
                setTempGoals({ ...weeklyGoals });
                setIsGoalModalVisible(true);
              }}
            >
              <Text style={styles.editGoalsHeaderButtonText}>Edit Weekly Volume</Text>
            </Pressable>
          </View>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeaderRow}>
            ...
            <View style={styles.prUnitSelector}>
              {(["kg", "lb"] as WeightUnit[]).map((unit) => (
                <Pressable key={unit} onPress={() => changePrUnit(unit)} style={[styles.prUnitButton, prUnit === unit && styles.prUnitButtonActive]}>
                  <Text style={[styles.prUnitButtonText, prUnit === unit && styles.prUnitButtonTextActive]}>{unit.toUpperCase()}</Text>
                </Pressable>
              ))}
            </View>
          </View>
          ...
          <View style={styles.filterContainer}>
            <ScrollView
              ...
              contentContainerStyle={styles.modalityFilterRow}
              keyboardShouldPersistTaps="handled"
            >
              <Pressable
                style={[styles.filterChip, showFavoritesOnly && styles.chipActive]}
                onPress={() => setShowFavoritesOnly(!showFavoritesOnly)}
              >
                <Text style={[styles.filterChipText, showFavoritesOnly && styles.chipTextActive]}>Favorites</Text>
              </Pressable>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.muscleFilterScrollCentered}>
                        ...
                        <Text style={styles.prWeightText}>
                          {pr.isHold
                            ? pr.weightAtHold && pr.weightAtHold > 0
                              ? `${formatWeight(pr.weightAtHold, pr.sourceUnit)} × ${pr.holdSeconds || pr.reps} sec`
                              : `${pr.holdSeconds || pr.reps} sec`
                            : pr.maxWeight > 0
                            ? formatWeight(pr.maxWeight, pr.sourceUnit)
                            : `${pr.reps} reps`}
                        </Text>
                        <Text style={styles.prSubtext}>
                          {pr.isHold
                            ? pr.progressionParent
                              ...
                              : `Longest hold`
                            : pr.maxWeight > 0
                            ? `for ${pr.reps} reps • Est. 1RM: ${formatWeight(pr.estimated1RM || 0, pr.sourceUnit)}`
                            : `Top rep set`}
                        </Text>
                      </Pressable>
                    );
                      ...
                      return (
                        <View key={index} style={styles.setRow}>
                          ...
                          <Text style={styles.setMetricsText}>
                            {historyIsHold
                              ? setObj.load
                                ? `${formatWeight(Number(setObj.load), setObj.loadUnit || "kg")} • ${setObj.reps || "0"} sec`
                                : `${setObj.reps || "0"} sec`
                              : setObj.load
                              ? `${formatWeight(Number(setObj.load), setObj.loadUnit || "kg")} × ${setObj.reps || "0"}`
                              : `0 ${prUnit} × ${setObj.reps || "0"}`}
            ...
            <ScrollView contentContainerStyle={styles.modalScrollBody} showsVerticalScrollIndicator={false}>
              ...
              {selectedExerciseIsHold && graphMode === "holdAtWeight" && (
                <View style={styles.repInputWrapper}>
                  <NumericInput
                    style={styles.repTextInput}
                    placeholder={`Enter target weight (${prUnit})...`}
                    placeholderTextColor={theme.colors.textSecondary}
                  ...
                  <View style={styles.modalHighlightsGrid}>
                      ...
                      <Text style={styles.highlightValue}>
                        {selectedExerciseIsHold
                          ? graphMode === "holdAtWeight" || graphMode === "maxHold"
                            ...
                            : formatWeight(listProgressionData[0]?.weight || 0)
                    ...
                    <View style={styles.highlightBlock}>
                      ...
                      <Text style={[styles.highlightValue, { color: theme.colors.accent }]}> 
                        {selectedExerciseIsHold
                          ? formatWeight(listProgressionData[0]?.weight || 0)
                          : formatWeight(listProgressionData[0]?.estimated1RM || 0)}
                      </Text>
                    </View>
                  </View>
                          ...
                          <Text style={styles.svgNodeWeightText}>
                            {selectedExerciseIsHold
                              ? (graphMode === "weightAtHold" ? `${cp.pt.weight}kg` : `${cp.pt.reps}s`)
                                : graphMode === "repsAtWeight" ? `${cp.pt.reps} reps` : formatWeight(cp.pt.weight)}
                          </Text>
                        </View>
                      ))}
                      ...
                      <View key={mIdx} style={styles.milestoneLogCard}>
                        <View style={{ flex: 1 }}>
                          <Text style={styles.milestoneMainText}>
                            {selectedExerciseIsHold
                              ? milestone.weight > 0 ? `${formatWeight(milestone.weight)} × ${milestone.reps} sec` : `${milestone.reps} sec`
                              : graphMode === "repsAtWeight" ? `${milestone.reps} reps` : `${formatWeight(milestone.weight)} × ${milestone.reps} reps`}
                          </Text>
                        ...
                        {!selectedExerciseIsHold && (
                          <View style={{ alignItems: "flex-end" }}>
                            ...
                            <Text style={styles.milestone1RMValue}>{formatWeight(milestone.estimated1RM || 0, "kg")}</Text>
                          </View>
                        )}
                      </View>
                    ))}
      ...
      <Modal visible={isProfileModalVisible} transparent animationType="slide" onRequestClose={() => setIsProfileModalVisible(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.profileModal}>
            <View style={styles.sectionHeaderRow}><Text style={styles.modalTitle}>Edit training profile</Text><Pressable onPress={() => setIsProfileModalVisible(false)}><Text style={styles.closeText}>Close</Text></Pressable></View>
            {profileDraft && <ScrollView contentContainerStyle={styles.profileForm}>
              <Text style={styles.profileFieldLabel}>Intent</Text>
              <View style={styles.profileChoiceRow}>{(["calisthenics", "weights", "hybrid"] as TrainingModality[]).map((value) => <Pressable key={value} onPress={() => setProfileDraft({ ...profileDraft, modality: value })} style={[styles.profileChoice, profileDraft.modality === value && styles.profileChoiceActive]}><Text style={[styles.profileChoiceText, profileDraft.modality === value && styles.profileChoiceTextActive]}>{value === "weights" ? "Weights" : value === "calisthenics" ? "Cali" : "Hybrid"}</Text></Pressable>)}</View>
              <Text style={styles.profileFieldLabel}>Training emphasis</Text>
              <View style={styles.profileChoiceRow}>{(["skills", "hypertrophy", "strength", "explosiveness", "mixed"] as TrainingStyle[]).map((value) => <Pressable key={value} onPress={() => setProfileDraft({ ...profileDraft, trainingStyle: value })} style={[styles.profileChoice, profileDraft.trainingStyle === value && styles.profileChoiceActive]}><Text style={[styles.profileChoiceText, profileDraft.trainingStyle === value && styles.profileChoiceTextActive]}>{value === "mixed" ? "Mix" : value.charAt(0).toUpperCase() + value.slice(1)}</Text></Pressable>)}</View>
              {profileDraft.modality !== "weights" && <><Text style={styles.profileFieldLabel}>Skill focus (optional)</Text><View style={styles.profileChoiceRow}>{(["frontLever", "planche", "handstand", "muscleUp"] as SkillFocus[]).map((value) => <Pressable key={value} onPress={() => setProfileDraft({ ...profileDraft, skillFocus: profileDraft.skillFocus === value ? undefined : value })} style={[styles.profileChoice, profileDraft.skillFocus === value && styles.profileChoiceActive]}><Text style={[styles.profileChoiceText, profileDraft.skillFocus === value && styles.profileChoiceTextActive]}>{value === "frontLever" ? "Front lever" : value === "muscleUp" ? "Muscle-up" : value.charAt(0).toUpperCase() + value.slice(1)}</Text></Pressable>)}</View></>}
              <Text style={styles.profileFieldLabel}>Weight units</Text>
              <View style={styles.profileChoiceRow}>{(["kg", "lb"] as const).map((value) => <Pressable key={value} onPress={() => setProfileDraft({ ...profileDraft, weightUnit: value })} style={[styles.profileChoice, profileDraft.weightUnit === value && styles.profileChoiceActive]}><Text style={[styles.profileChoiceText, profileDraft.weightUnit === value && styles.profileChoiceTextActive]}>{value.toUpperCase()}</Text></Pressable>)}</View>
              <Text style={styles.profileFieldLabel}>Height units</Text>
              <View style={styles.profileChoiceRow}>{(["cm", "ft"] as const).map((value) => <Pressable key={value} onPress={() => setProfileDraft({ ...profileDraft, heightUnit: value })} style={[styles.profileChoice, profileDraft.heightUnit === value && styles.profileChoiceActive]}><Text style={[styles.profileChoiceText, profileDraft.heightUnit === value && styles.profileChoiceTextActive]}>{value === "ft" ? "FT / IN" : "CM"}</Text></Pressable>)}</View>
              <View style={styles.profileEditInputRow}><Text style={styles.profileFieldLabel}>Bodyweight</Text><NumericInput value={String(profileDraft.weight || "")} onChangeText={(value) => setProfileDraft({ ...profileDraft, weight: Number(value) || 0 })} style={styles.profileInput} placeholder="0" placeholderTextColor={theme.colors.textMuted} allowDecimal label="Bodyweight" /></View>
              <View style={styles.profileEditInputRow}><Text style={styles.profileFieldLabel}>Height ({profileDraft.heightUnit})</Text><NumericInput value={String(profileDraft.height || "")} onChangeText={(value) => setProfileDraft({ ...profileDraft, height: Number(value) || 0 })} style={styles.profileInput} placeholder="0" placeholderTextColor={theme.colors.textMuted} allowDecimal label="Height" /></View>
              {profileDraft.heightUnit === "ft" && <View style={styles.profileEditInputRow}><Text style={styles.profileFieldLabel}>Extra inches</Text><NumericInput value={String(profileDraft.heightInches || "")} onChangeText={(value) => setProfileDraft({ ...profileDraft, heightInches: Number(value) || 0 })} style={styles.profileInput} placeholder="0" placeholderTextColor={theme.colors.textMuted} allowDecimal label="Extra inches" /></View>}
              <Button fullWidth onPress={saveProfileDraft}>Save profile</Button>
            </ScrollView>}
          </View>
        </View>
      </Modal>
      <Modal visible={showGoalCompletion} transparent animationType="fade" onRequestClose={() => setShowGoalCompletion(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.profileEyebrow}>GOAL REACHED</Text>
            <Text style={styles.modalTitle}>{completedGoalTitles.join(" and ")}</Text>
            <Text style={styles.modalDetail}>
              You hit an automatically tracked target. It has been recorded and removed from your active goals.
            </Text>
            <Button fullWidth onPress={() => setShowGoalCompletion(false)}>Keep progressing</Button>
          </View>
        </View>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  profileCard: { backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 14, padding: theme.spacing.md, gap: theme.spacing.md },
  profileCardHeader: { flexDirection: "row", alignItems: "center", gap: 12 },
  profileEyebrow: { ...theme.typography.caption, color: theme.colors.accent, fontSize: 11, fontWeight: "800", letterSpacing: 0.6 },
  profileTitle: { ...theme.typography.heading, color: theme.colors.textPrimary, marginTop: 2 },
  profileSubtitle: { ...theme.typography.caption, color: theme.colors.textSecondary, marginTop: 3 },
  profileSignals: { flexDirection: "row", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: theme.spacing.sm },
  profileSignal: { ...theme.typography.caption, color: theme.colors.textSecondary, fontSize: 11 },
  goalsPanel: { backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 14, padding: theme.spacing.md, gap: theme.spacing.sm },
  recommendationHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: theme.spacing.sm, paddingBottom: theme.spacing.xs },
  goalsHeaderToggle: { flexDirection: "row", alignItems: "center", flex: 1, gap: theme.spacing.sm },
  goalsCollapseIcon: { color: theme.colors.accent, fontSize: 22, fontWeight: "700", paddingHorizontal: 2 },
  recommendationTitle: { ...theme.typography.heading, color: theme.colors.textPrimary, fontSize: 20, lineHeight: 25 },
  recommendationCount: { color: theme.colors.accent, fontSize: 18, fontWeight: "800" },
  recommendationRow: { borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: theme.spacing.sm, gap: 3 },
  recommendationItemTitle: { ...theme.typography.body, color: theme.colors.textPrimary, fontWeight: "700" },
  recommendationDetail: { ...theme.typography.caption, color: theme.colors.textSecondary },
  recommendationAction: { ...theme.typography.caption, color: theme.colors.accent, fontWeight: "700", marginTop: 2 },
  programmedGoalRow: { borderTopWidth: 1, borderTopColor: theme.colors.border, paddingTop: theme.spacing.sm, gap: 5 },
  programmedGoalHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: theme.spacing.sm },
  goalValue: { ...theme.typography.caption, color: theme.colors.accent, fontWeight: "700" },
  goalTrack: { height: 6, backgroundColor: theme.colors.background, borderRadius: 3, overflow: "hidden" },
  goalFill: { height: "100%", backgroundColor: theme.colors.accent, borderRadius: 3 },
  goalTipsBanner: { backgroundColor: theme.colors.surfaceRaised, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 10, padding: 11, gap: 3 },
  goalTipsTitle: { color: theme.colors.accent, fontSize: 11, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.5 },
  goalTipsText: { color: theme.colors.textSecondary, fontSize: 12, lineHeight: 17 },
  prUnitSelector: { flexDirection: "row", gap: 5 },
  prUnitButton: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 7, borderWidth: 1, borderColor: theme.colors.border, backgroundColor: theme.colors.surface },
  prUnitButtonActive: { backgroundColor: theme.colors.accent, borderColor: theme.colors.accent },
  prUnitButtonText: { color: theme.colors.textSecondary, fontSize: 10, fontWeight: "800" },
  prUnitButtonTextActive: { color: theme.colors.background },
  modalBackdrop: { flex: 1, backgroundColor: "rgba(0, 0, 0, 0.6)", justifyContent: "center", alignItems: "center", padding: theme.spacing.md },
  profileModal: { maxHeight: "85%", width: "100%", maxWidth: 360, backgroundColor: theme.colors.surface, borderRadius: 16, borderWidth: 1, borderColor: theme.colors.border, padding: theme.spacing.lg, gap: theme.spacing.md, elevation: 5 },
  modalCard: { width: "100%", maxWidth: 360, backgroundColor: theme.colors.surface, borderRadius: 16, borderWidth: 1, borderColor: theme.colors.border, padding: theme.spacing.lg, gap: theme.spacing.md, elevation: 5 },
  modalDetail: { ...theme.typography.body, color: theme.colors.textSecondary, lineHeight: 20 },
  closeText: { color: theme.colors.textSecondary, fontSize: 13, fontWeight: "600" },
  profileForm: { gap: theme.spacing.md, paddingBottom: theme.spacing.sm },
  profileFieldLabel: { ...theme.typography.caption, color: theme.colors.textSecondary, fontWeight: "600" },
  profileChoiceRow: { flexDirection: "row", flexWrap: "wrap", gap: theme.spacing.sm },
  profileChoice: { borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8, paddingVertical: 9, paddingHorizontal: 12, backgroundColor: theme.colors.surfaceRaised },
  profileChoiceActive: { borderColor: theme.colors.accent, backgroundColor: theme.colors.accent },
  profileChoiceText: { ...theme.typography.caption, color: theme.colors.textPrimary, fontWeight: "600" },
  profileChoiceTextActive: { color: "#FFFFFF", fontWeight: "700" },
  profileEditInputRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  profileInput: { minWidth: 100, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8, color: theme.colors.textPrimary, padding: 10, textAlign: "right" },
  ...
  modalityFilterRow: { flexDirection: "row", alignItems: "center", gap: 6, paddingRight: 12 },
  muscleFilterScroll: { flexDirection: "row", alignItems: "center", gap: 6 },
  muscleFilterScrollCentered: { flexGrow: 1, justifyContent: "center", alignItems: "center", gap: 6, paddingHorizontal: 8 },
  ...
  filterChip: { flexShrink: 0, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border },
