...
const ONBOARDING_QUESTS_KEY = "@onboarding_quests";
...
export function RoutinesScreen() {
  ...
  const handleSaveRoutine = async () => {
    ...
    const isEditingStarter = activeStarters.some(r => r.id === editingRoutineId);
    ...
    const questKey = routineDescription.includes("routine targeting") ? "generateRoutine" : "customRoutine";
    const questState = JSON.parse((await AsyncStorage.getItem(ONBOARDING_QUESTS_KEY)) || "{}");
    await AsyncStorage.setItem(ONBOARDING_QUESTS_KEY, JSON.stringify({ ...questState, [questKey]: true }));
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView contentContainerStyle={styles.builderScrollBody} style={{ maxHeight: 420 }} showsVerticalScrollIndicator={false}>
...
const styles = StyleSheet.create({
  ...
  profileHint: { color: theme.colors.textSecondary, fontSize: 12, lineHeight: 17, marginTop: -6 },
...
deleteButtonText: {
  ...
  builderScrollBody: { gap: 10, paddingBottom: 8 },
  chip: {
    paddingHorizontal: 12,
    paddingVertical: 6,
    borderRadius: 20,
