...
import { createStarterRoutines } from "../data/profileStarterRoutines";
import { exerciseLibrary } from "../data/exerciseLibrary";
import { muscleGroups } from "../data/muscleGroups";
import { theme } from "../theme/theme";
import { useRoutines } from "../context/RoutineContext";
import { useUserProfile } from "../context/UserProfileContext";
import { WeightUnit } from "../models/userProfile";
...
import {
  ...
  emptyStreakState,
  endDeload,
  invalidateRestDayCredit,
  invalidateTodayCredit,
  invalidateWorkoutCredit,
  restoreRestDayCredit,
  resolveStreak,
  startDeload,
  StreakState,
  STREAK_STATE_KEY,
...
const EXERCISE_LOAD_UNITS_KEY = "@exercise_load_units";
...
export function HomeScreen() {
  ...
  const shownStreakCelebrations = useRef(new Set<string>());
  ...
  const [streakLoadedForSplitId, setStreakLoadedForSplitId] = useState<string | null>(null);
...
// Current Workout Tracking State
  ...
  const [loadUnit, setLoadUnit] = useState<WeightUnit>(profile?.weightUnit || "kg");
  const [exerciseLoadUnits, setExerciseLoadUnits] = useState<Record<string, WeightUnit>>({});
  ...
  useEffect(() => {
    AsyncStorage.getItem(EXERCISE_LOAD_UNITS_KEY).then((stored) => {
      if (stored) setExerciseLoadUnits(JSON.parse(stored));
    }).catch((error) => console.error("Failed to load exercise weight units", error));
  }, []);
  ...
  // Computed Lists
  const activeStarters = createStarterRoutines(profile || { modality: "hybrid", trainingStyle: "mixed", trainingDays: 3, weightUnit: "kg", heightUnit: "cm", weight: 0, height: 0, onboardingComplete: true }).filter((sr) => !deletedStarterIds?.includes(sr.id));
  ...
  const streakStorageKey = activeSplitId ? `${STREAK_STATE_KEY}:${activeSplitId}` : null;

  const persistStreakState = useCallback(async (next: StreakState) => {
    setStreakState(next);
    if (!streakStorageKey) return;
    try {
      await AsyncStorage.setItem(streakStorageKey, JSON.stringify(next));
    } catch (e) {
  ...
  useEffect(() => {
    const loadStreak = async () => {
      if (!streakStorageKey) {
        setStreakState(emptyStreakState);
        setStreakLoadedForSplitId(null);
        return;
      }

      setStreakLoadedForSplitId(null);
      try {
        let saved = await AsyncStorage.getItem(streakStorageKey);
        if (!saved) {
          const legacySaved = await AsyncStorage.getItem(STREAK_STATE_KEY);
          if (legacySaved) {
            saved = legacySaved;
            await AsyncStorage.setItem(streakStorageKey, legacySaved);
        ...
        if (JSON.stringify(resolved) !== JSON.stringify(parsed)) {
          await AsyncStorage.setItem(streakStorageKey, JSON.stringify(resolved));
        }
        setStreakLoadedForSplitId(activeSplitId);
      } catch (e) {
    ...
    loadStreak();
  }, [activeSplitId, streakStorageKey]);

  // A day counts once every routine the split scheduled is done — a scheduled
  // rest day counts as following the split too.
  const isScheduledRestDay = !!activeSplit && todaysRoutineIds.length === 0;
  const followedSplitToday =
    !!activeSplit &&
    (isScheduledRestDay
      ? !streakState.requiresWorkoutToday
      : todaysRoutineIds.every((routineId) => completedRoutineIds.includes(routineId)));

  useEffect(() => {
    if (streakLoadedForSplitId !== activeSplitId || isScheduledRestDay || !streakState.creditedAsRestDay) return;
    const invalidated = invalidateRestDayCredit(streakState);
    if (invalidated !== streakState) persistStreakState(invalidated);
  }, [activeSplitId, isScheduledRestDay, streakLoadedForSplitId, streakState, persistStreakState]);

  useEffect(() => {
    if (!activeSplitId || streakLoadedForSplitId !== activeSplitId || !followedSplitToday || streakState.deloadActive) return;
    const credited = creditStreakDay(streakState, new Date(), isScheduledRestDay);
    if (credited === streakState) return;
    const streakPopupKey = `${activeSplitId}:${dateKey()}`;
    persistStreakState(credited);
    if (!shownStreakCelebrations.current.has(streakPopupKey)) {
      shownStreakCelebrations.current.add(streakPopupKey);
      enqueueCelebration("streak", `Day ${credited.streak} — today's split is done.`);
    }
  }, [activeSplitId, followedSplitToday, isScheduledRestDay, streakLoadedForSplitId, streakState, persistStreakState, enqueueCelebration]);
 ...
 const normalizeExerciseKey = (rawName) => {
  ...
  const getLastLoggedPerformance = useCallback(
    (targetExerciseName: string) => {
      ...
      const performanceFromSets = (sets) => {
        const matches = (sets || []).filter(
          (set) => normalizeProgressionKey(set.exercise) === targetKey
        );
        const loggedSet = matches[0];
        if (!loggedSet) return null;
        return {
          load: loggedSet.load ? String(loggedSet.load) : "",
          unit: loggedSet.loadUnit || exerciseLoadUnits[targetKey] || profile?.weightUnit || "kg",
      ...
      const currentPerformance = performanceFromSets(setLog);
      ...
      if (workoutHistory && workoutHistory.length > 0) {
        for (const session of workoutHistory) {
          ...
          const pastPerformance = performanceFromSets(session.sets);
          if (pastPerformance) return pastPerformance;
        }
      }

      return null;
    },
    [exerciseLoadUnits, profile?.weightUnit, setLog, workoutHistory]
  );
  ...
  useEffect(() => {
    ...
    setLoadUnit(lastPerf?.unit || exerciseLoadUnits[normalizeProgressionKey(exerciseToMatch)] || profile?.weightUnit || "kg");
  ...
  const computeWorkoutSummaryMetrics = (sets) => {
    ...
    (sets || []).forEach((item) => {
      const loadValue = loadInKg(item.load, item.loadUnit || "kg");
  ...
  const loadInKg = (value, unit = "kg") => {
    const parsed = parseFloat(value) || 0;
    return unit === "lb" ? parsed / 2.20462262 : parsed;
  };
  const displayLoad = (value, unit = loadUnit) => {
    const kg = loadInKg(value, unit);
    const converted = unit === "lb" ? kg * 2.20462262 : kg;
    return `${Math.round(converted * 10) / 10} ${unit}`;
  };
  const displayKgInActiveUnit = (kg) => {
    const converted = loadUnit === "lb" ? kg * 2.20462262 : kg;
    return `${Math.round(converted * 10) / 10} ${loadUnit}`;
  };
  ...
  const computePrReplacementForSet = (exerciseName, loadValue, repsValue, isHoldSet, currentSetLog = []) => {
    ...
    const normalizedLoad = loadInKg(loadValue, loadUnit);
    ...
    const loadOf = (set) => loadInKg(set.load, set.loadUnit || "kg");
    ...
    if (isStaticHold) {
      ...
      if (normalizedLoad > 0 && bestHoldAtWeight > 0 && normalizedReps > bestHoldAtWeight) {
        records.push(`Max hold time at ${displayLoad(loadValue)}: ${normalizedReps}s (was ${bestHoldAtWeight}s)`);
      }
      if (normalizedLoad > 0 && bestWeightAtHold > 0 && normalizedLoad > bestWeightAtHold) {
        records.push(`Max weight at ${normalizedReps}s hold: ${displayLoad(loadValue)} (was ${displayKgInActiveUnit(bestWeightAtHold)})`);
      }
    } else {
      ...
      if (bestWeight > 0 && normalizedLoad > bestWeight) {
        records.push(`Max weight: ${displayLoad(loadValue)} (was ${displayKgInActiveUnit(bestWeight)})`);
      }
      if (bestWeightAtReps > 0 && normalizedLoad > bestWeightAtReps) {
        records.push(`Max weight at ${normalizedReps} reps: ${displayLoad(loadValue)} (was ${displayKgInActiveUnit(bestWeightAtReps)})`);
      }
      if (bestRepsAtWeight > 0 && normalizedReps > bestRepsAtWeight) {
        records.push(`Max reps at ${displayLoad(loadValue)}: ${normalizedReps} reps (was ${bestRepsAtWeight} reps)`);
      }
    }
  ...
  const submitLog = async () => {
    ...
    const exerciseKey = normalizeProgressionKey(exerciseToLog);
    const nextExerciseLoadUnits = { ...exerciseLoadUnits, [exerciseKey]: loadUnit };
    setExerciseLoadUnits(nextExerciseLoadUnits);
    AsyncStorage.setItem(EXERCISE_LOAD_UNITS_KEY, JSON.stringify(nextExerciseLoadUnits)).catch((error) => console.error("Failed to save exercise weight unit", error));
    setSelectedSubSkill(null);
    setLoad("");
    setReps("");
    setRir("");
  ...
  const addRoutineToDay = (routineId) => {
    ...
    const updatedDays = activeSplit.days.map((day) => ({
      ...day,
    ...
    // A rest day was already credited, but adding work to today means that
    // credit must be reclaimed by completing the newly scheduled routine.
    if (targetDayIndex === currentSplitDayIndex && streakState.lastCreditedDate === dateKey()) {
      persistStreakState(invalidateTodayCredit(streakState));
    }
    setIsSearchModalVisible(false);
  };
  ...
  const removeRoutineFromDay = (dayIdx, routineIdx) => {
    ...
    const removedRoutineId = activeSplit.days?.[dayIdx]?.routineIds?.[routineIdx];
    const updatedDays = activeSplit.days.map((day) => ({
      ...day,
      routineIds: [...(day.routineIds || [])],
    }));
    ...
    if (dayIdx === currentSplitDayIndex && removedRoutineId && completedRoutineIds.includes(removedRoutineId)) {
      const nextCompletedIds = completedRoutineIds.filter((id) => id !== removedRoutineId);
      setCompletedRoutineIds(nextCompletedIds);
      AsyncStorage.setItem(
        COMPLETED_ROUTINES_TODAY_KEY,
        JSON.stringify({
          date: new Date().toDateString(),
      ...
      const nextStreakState = updatedDays[dayIdx].routineIds.length === 0
        ? restoreRestDayCredit(invalidateWorkoutCredit(streakState))
        : invalidateWorkoutCredit(streakState);
      persistStreakState(nextStreakState);
    } else if (dayIdx === currentSplitDayIndex && updatedDays[dayIdx].routineIds.length === 0) {
      persistStreakState(restoreRestDayCredit(streakState));
    }
  };
          ...
          <View style={styles.celebrationRow}>
            {celebrations
              ...
              .map((item) => (
                <View
                  key={item.id}
                  style={[styles.celebrationCardWrapper, item.variant === 'workoutFinish' && styles.workoutFinishWrapper]}
                >
            ...
            <View style={styles.header}>
              {activeSplit ? (
                <View style={styles.inlineSplitHeaderContainer}>
                  ...
                  <Pressable style={[styles.deloadButton, streakState.deloadActive && styles.deloadButtonActive]} onPress={toggleDeload}>
                  ...
                  <View style={styles.streakCorner}>
                    <FlameIcon size={22} opacity={followedSplitToday && !streakState.deloadActive ? 1 : 0.3} />
                    <Text style={[styles.streakValue, followedSplitToday && !streakState.deloadActive && styles.streakValueActive]}>
                      {streakState.streak}
                    </Text>
                  </View>
                </View>
              ...
              {activeSplit?.days.map((dayObj, dayIdx) => (
                <View key={dayObj.day} style={styles.splitDayConfigCard}>
                  ...
                  <View style={styles.splitDayRoutinesList}>
                    {(() => {
                      const visibleRoutineIds = (dayObj.routineIds || []).filter((id) => allRoutines.some((routine) => routine.id === id));
                      return visibleRoutineIds.length === 0 ? (
                      <View style={styles.restDayPlaceholderBox}>
                        <Text style={styles.restDayLabelText}>Rest Day</Text>
                      </View>
                    ) : (
                      visibleRoutineIds.map((id, rIdx) => {
                ...
                {phase === "logging" && (
                  <View style={styles.logBox}>
                    ...
                    <View style={styles.loadInputRow}>
                      <NumericInput placeholder={`Load (${loadUnit})`} value={load} onChangeText={setLoad} style={styles.input} placeholderTextColor={theme.colors.textSecondary} maxLength={4} allowDecimal label={`Load (${loadUnit})`} />
                      <View style={styles.unitSelector}>
                        {(["kg", "lb"] as WeightUnit[]).map((unit) => <Pressable key={unit} onPress={() => setLoadUnit(unit)} style={[styles.unitButton, loadUnit === unit && styles.unitButtonActive]}><Text style={[styles.unitButtonText, loadUnit === unit && styles.unitButtonTextActive]}>{unit.toUpperCase()}</Text></Pressable>)}
                      </View>
                    </View>
...
const styles = StyleSheet.create({
  ...
  unitButton: { flex: 1, alignItems: "center", paddingVertical: 9, borderRadius: 8, borderWidth: 1, borderColor: theme.colors.border, backgroundColor: theme.colors.surface },
  unitButtonActive: { backgroundColor: theme.colors.accent, borderColor: theme.colors.accent },
  unitButtonText: { color: theme.colors.textSecondary, fontSize: 12, fontWeight: "800" },
  unitButtonTextActive: { color: theme.colors.background },
  ...
  workoutFinishWrapper: { position: "absolute", left: 0, right: 0, top: 0, bottom: 0, minWidth: 0, maxWidth: undefined },
  ...
  routineSearchInput: { padding: 12, borderRadius: 8, backgroundColor: theme.colors.background || "#111", borderWidth: 1, borderColor: theme.colors.border, color: theme.colors.textPrimary, fontSize: 14, marginBottom: 12 },
  routineFilterContainer: { marginHorizontal: -4, marginBottom: 12 },
  routineFilterScroll: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 4, paddingVertical: 2 },
  ...
  routineExerciseSelectorList: { maxHeight: 200, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8, backgroundColor: theme.colors.background || "#111", marginBottom: 14 },
  ...
  exercisePickerFooter: { marginTop: 2 },
